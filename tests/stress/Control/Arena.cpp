...
BOOST_AUTO_TEST_CASE(RandomWalk) {
  ...
  for (int loops = 0; loops < 3; ++loops) {
    for (int i = 0; i < nblocks; ++i)
      BOOST_REQUIRE(verifyAcquire(arena.acquire<int>()));
    BOOST_REQUIRE(dirty.find(reinterpret_cast<std::uintptr_t>(nullptr)) ==
                  dirty.end());
    BOOST_REQUIRE(nullptr == arena.acquire<int>());
    {
      ...
      for (int i = 0; i < nblocks - 1; ++i)
        BOOST_CHECK(sizeof(Arena::Block) <= dvec[i + 1] - dvec[i]);
    }
