...
namespace Profiler { namespace Control { namespace Test
{
    ...
    BOOST_AUTO_TEST_CASE(Small)
    {
        ...
        auto verifyAcquire = [&dirty](void* ptr_) {
            // Verify that the just acquired address doens't conflict with what was previously acquired.
            auto iptr = reinterpret_cast<std::uintptr_t>(ptr_);
            if (dirty.find(iptr) != dirty.end()) return 1;
            dirty.insert(iptr);
            auto next = dirty.upper_bound(iptr);
            if (next != dirty.end() && *next < iptr + sizeof(Arena::Block)) return 2;
            if (iptr < sizeof(Arena::Block)) return 0;
            auto prev = dirty.lower_bound(iptr - sizeof(Arena::Block));
            if (*prev != iptr && iptr < *prev + sizeof(Arena::Block)) return 3;
            return 0;
        };
