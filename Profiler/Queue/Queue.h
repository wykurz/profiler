...
template <typename T_> struct Queue {
  using Type = T_;
  ...
  struct Node {
    ...
    Node(Node &&) = delete;
    Node *getNext() const { return _next.load(std::memory_order_acquire); }
  ...
  Node *extract();
  std::size_t size()
      const; // Assumes the queue is not modified for the duration of the call.
...
template <typename T_> typename Queue<T_>::Node *Queue<T_>::head() const {
  return _head.load(std::memory_order_acquire);
}

template <typename T_> void Queue<T_>::push(Node *node_) {
  node_->updateTag();
  ...
  do {
    node_->setNext(unpackPtr(headPtr));
      ...
      headPtr, nodePtr, std::memory_order_release, std::memory_order_relaxed));
}

template <typename T_> typename Queue<T_>::Node *Queue<T_>::pull() {
...
template <typename T_> typename Queue<T_>::Node *Queue<T_>::extract() {
...
template <typename T_> std::size_t Queue<T_>::size() const {
  // Should be used for testing purposes only. It's worth remembering that
  // computing the size of a lock-free list
  // this way may results in a number of elements that this list had never
  // contained at any point in time.
  std::size_t size = 0;
