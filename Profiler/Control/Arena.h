...
struct Arena {
  ...
  template <typename T_> T_ *acquire() {
    static_assert(sizeof(T_) <= DataSize,
                  "Blocks is to small to hold the requested object.");
    static_assert(
        alignof(Block) % alignof(T_) == 0,
        "Block alignement not congruent to requested type alignment.");
  ...
  template <typename T_> void release(T_ *block_) {
    // Technically we don't need to require a typed pointer and could use void*,
  ...
  /**
   * Base pointer for a given type T. All Block-s in Arena will have a positive
   * offset from the base pointer.
   *
   * Base pointer is guaranteed to never change during the lifetime of the
   * arena.
   */
  char *basePtr() { return static_cast<char *>(_buffer.data()); }
...
private:
  ...
  int getIndex(void *ptr_) const {
    const auto offset = reinterpret_cast<std::intptr_t>(ptr_) -
                        reinterpret_cast<std::intptr_t>(_buffer.data());
